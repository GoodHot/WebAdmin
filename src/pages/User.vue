<template>
  <div>
    <a-button class="editable-add-btn" @click="showAddModel">新建用户</a-button>
    <a-table :columns="columns" :dataSource="data" :pagination="pagination" @change="handleTableChange">
      ...
      <span slot="staff" slot-scope="col">
        <a-tag color="#f50" v-if="col.staff">内部账号</a-tag>
        <a-tag color="#2db7f5" v-else>普通账号</a-tag>
      </span>
    ...
    <a-modal title="新建用户" v-model="visible" @ok="addAdmin">
      <a-form >
        ...
        <a-form-item label="是否内部账号" :labelCol="{ span: 6 }" :wrapperCol="{ span: 14 }">
          <a-switch @change="switchStaff"/>
        </a-form-item>
...
export default {
  ...
  methods: {
    getAdmin() {
      let self = this;
      api.pager('/user/page/', 1, function(ds, pg, change){
        self.data =ds;
        self.pagination = pg;
    ...
    addAdmin() {
      ...
      http.post('/user/add-offical', this.edit)
          .then(function(resp){
